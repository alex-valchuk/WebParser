...
namespace Searcher.Lib
{
    public class ShopProductFactory : ProductFactory
    {
        ...
        /// </summary>
        /// <param name="searchArea"> area of searching for </param>
        /// <param name="searchItem"> item of searching for </param>
        /// <returns> IList<String> </returns>
        internal override IList<String> MakeSubPathList(ISearchArea searchArea, ISearchItem searchItem)
        {
            ShopItem item = searchItem as ShopItem;
            ShopArea area = searchArea as ShopArea;
            area.Text = RegexHelper.ReplacePatternFromText(area.Text, item.ExcludedPathList);
            return RegexHelper.GetSubAddressList(area.Path, area.Text);
        }
        ...
        private ShopProduct GetShopProduct(ShopItem item, String text)
        {
            ...
            if (!String.IsNullOrEmpty(altPrice))
            {
